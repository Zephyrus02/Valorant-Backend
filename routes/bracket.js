...
const createNextRound = (bracket, currentRoundNumber) => {
	...
	for (let i = 0; i < currentRound.matchups.length; i += 2) {
		const newMatchup = {
			...
			team2: currentRound.matchups[i + 1]
				? currentRound.matchups[i + 1].winner
				: null,
			winner: null,
		};
...
const updateWinnerAndCreateNextRound = async (
	bracket,
	roundNumber,
	matchId,
	winnerName
...
router.post("/initialize", authMiddleware, async (req, res) => {
	try {
		if (req.user.role !== "admin") {
			return res
				.status(403)
				.json({ msg: "Only admins can initialize brackets" });
		}
		...
		res
			.status(201)
			.json({ msg: "Bracket initialized successfully", bracket: newBracket });
	} catch (err) {
...
router.post("/update", authMiddleware, async (req, res) => {
	try {
		...
		await updateWinnerAndCreateNextRound(
			bracket,
			roundNumber,
			matchId,
			winnerName
		...
		res.json({
			msg: "Match winner updated and next round created if necessary.",
			bracket,
		});
	} catch (err) {
		res.status(500).json({ msg: "Server error", error: err.message });
