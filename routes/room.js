...
router.post('/join', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (room.participants.length === 2) {
            ...
            await room.save();
            return res.json({ msg: 'Game started. Both participants have joined.', room });
        }

        res.json({ msg: 'Successfully joined the room. Waiting for another participant.', room });
    } catch (err) {
        res.status(500).json({ msg: 'Server error', error: err.message });
    }
});
...
router.post('/select', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        await room.save();
        res.json({ msg: 'Title selected successfully', remainingTitles: room.titles });
    } catch (err) {
        res.status(500).json({ msg: 'Server error', error: err.message });
    }
});
...
router.post('/choice', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (room.titles.length > 1) {
            return res.status(400).json({ msg: 'You can only make this choice after the title selection is complete.' });
        }
        ...
        if (!['attacker', 'defender'].includes(choice)) {
            return res.status(400).json({ msg: 'Choice must be attacker or defender.' });
        }
